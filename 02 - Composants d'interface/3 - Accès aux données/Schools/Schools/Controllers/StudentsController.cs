...
namespace Schools.Controllers
{
    ...
    public class StudentsController : ControllerBase
    {
        ...
        //GET api/Students
        [HttpGet]
        public ActionResult<IEnumerable<StudentDTOOut>> GetAllStudents()
        {
            IEnumerable<Student> listeStudents = _service.GetAllStudents();
            return Ok(_mapper.Map<IEnumerable<StudentDTOOut>>(listeStudents));
        }

        //GET api/Students/{i}
        [HttpGet("{id}", Name = "GetStudentById")]
        public ActionResult<StudentDTOOut> GetStudentById(int id)
        {
            Student commandItem = _service.GetStudentsById(id);
            if (commandItem != null)
            {
                return Ok(_mapper.Map<StudentDTOOut>(commandItem));
            }
            return NotFound();
        }
